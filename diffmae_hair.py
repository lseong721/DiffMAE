...
def main(args, dataloader, model, optim, lr_scheduler, device):
    ...
    for epoch in range(args.total_epoch):
        ...
        for step, batch in enumerate(dataloader):
            ...
            # loss = criterion(patch_pred, patch_input)
        ...
        if epoch % 10 == 0:
            ''' save model '''
            torch.save(model, os.path.join(args.model_dir, '{}.pt'.format(args.log_name)))
            model.eval()
            with torch.no_grad():
                ...
                for idx, t in tqdm(enumerate(noise_scheduler.timesteps[0::100])):
                    if idx == 0:
                        outputs = model(img_clean, img_noise, t.to(device))
                    else:
                        outputs = model(img_clean, img_noise, t.to(device), noise=noise_seed)
                    ...
                    mask = mask.unsqueeze(-1).repeat(1, 1, model.config.patch_size**2 *img_pred.shape[1])  # (N, H*W, p*p*3)
                    mask = model.unpatchify(mask)  # 1 is removing, 0 is keeping

                    noise_seed = outputs.noise.detach()

                    # 2. compute previous image: x_t -> x_t-1
                    img_noise = noise_scheduler.add_noise(img_pred, img_noise, t)
                    img_noise = img_clean * (1 - mask) + img_noise * (mask)

                    # img_save = img_noise[0, :3].permute(1, 2, 0).detach().cpu().numpy()
                    ...
                    torch.save(img_pred_save.detach().cpu(), 'results/img_pred_nor_%03d.pt' % idx)
                    torch.save(img_clean_save.detach().cpu(), 'results/img_clean_nor_%03d.pt' % idx)
                img_pred = img_pred[:, :3]
                img_noise = img_noise[:, :3]
                img_clean = img_clean[:, :3]
                noise = noise[:, :3]
                mask = mask[:, :3]
                writer.add_images('mae_image', (img_noise.detach().cpu() + 1) / 2, epoch)
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('models/mae_hair.pt'))
    # model = torch.load('models/test_hair_nor.pt')
