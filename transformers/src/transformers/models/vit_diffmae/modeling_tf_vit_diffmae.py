...
@dataclass
class TFViTDiffMAEModelOutput(ModelOutput):
    """
    Class for TFViTDiffMAEModel's outputs, with potential hidden states and attentions.
...
@dataclass
class TFViTDiffMAEDecoderOutput(ModelOutput):
    """
    Class for TFViTDiffMAEDecoder's outputs, with potential hidden states and attentions.
...
@dataclass
class TFViTDiffMAEForPreTrainingOutput(ModelOutput):
    """
    Class for TFViTDiffMAEForPreTraining's outputs, with potential hidden states and attentions.
...
class TFViTDiffMAEEmbeddings(tf.keras.layers.Layer):
    """
    Construct the CLS token, position and patch embeddings.

    """

    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)

        self.patch_embeddings = TFViTDiffMAEPatchEmbeddings(config, name="patch_embeddings")
...
class TFViTDiffMAEPatchEmbeddings(tf.keras.layers.Layer):
    """
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTSelfAttention with ViT->ViTDiffMAE
class TFViTDiffMAESelfAttention(tf.keras.layers.Layer):
    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTSelfOutput with ViT->ViTDiffMAE
class TFViTDiffMAESelfOutput(tf.keras.layers.Layer):
    """
    The residual connection is defined in TFViTDiffMAELayer instead of here (as is the case with other models), due to the
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTAttention with ViT->ViTDiffMAE
class TFViTDiffMAEAttention(tf.keras.layers.Layer):
    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)

        self.self_attention = TFViTDiffMAESelfAttention(config, name="attention")
        self.dense_output = TFViTDiffMAESelfOutput(config, name="output")
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTIntermediate with ViT->ViTDiffMAE
class TFViTDiffMAEIntermediate(tf.keras.layers.Layer):
    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTOutput with ViT->ViTDiffMAE
class TFViTDiffMAEOutput(tf.keras.layers.Layer):
    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTLayer with ViT->ViTDiffMAE
class TFViTDiffMAELayer(tf.keras.layers.Layer):
    """This corresponds to the Block class in the timm implementation."""

    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)

        self.attention = TFViTDiffMAEAttention(config, name="attention")
        self.intermediate = TFViTDiffMAEIntermediate(config, name="intermediate")
        self.vit_output = TFViTDiffMAEOutput(config, name="output")
    ...
    def call(
        ...
        attention_outputs = self.attention(
            # in ViTDiffMAE, layernorm is applied before self-attention
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTEncoder with ViT->ViTDiffMAE
class TFViTDiffMAEEncoder(tf.keras.layers.Layer):
    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        super().__init__(**kwargs)

        self.layer = [TFViTDiffMAELayer(config, name=f"layer_._{i}") for i in range(config.num_hidden_layers)]
...
@keras_serializable
class TFViTDiffMAEMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: ViTDiffMAEConfig, **kwargs):
        ...
        self.embeddings = TFViTDiffMAEEmbeddings(config, name="embeddings")
        self.encoder = TFViTDiffMAEEncoder(config, name="encoder")
    ...
    def call(
        ...
        training: bool = False,
    ) -> Union[TFViTDiffMAEModelOutput, Tuple[tf.Tensor]]:
        ...
        return TFViTDiffMAEModelOutput(
...
class TFViTDiffMAEPreTrainedModel(TFPreTrainedModel):
    """
...
class TFViTDiffMAEModel(TFViTDiffMAEPreTrainedModel):
    def __init__(self, config: ViTDiffMAEConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)

        self.vit = TFViTDiffMAEMainLayer(config, name="vit")
    ...
    @replace_return_docstrings(output_type=TFViTDiffMAEModelOutput, config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        pixel_values: TFModelInputType | None = None,
        noise: tf.Tensor = None,
        ...
        ```python
        >>> from transformers import AutoImageProcessor, TFViTDiffMAEModel
        ...
        >>> model = TFViTDiffMAEModel.from_pretrained("facebook/vit-mae-base")
...
class TFViTDiffMAEDecoder(tf.keras.layers.Layer):
    def __init__(self, config, num_patches, **kwargs):
        ...
        self.decoder_layers = [
            TFViTDiffMAELayer(decoder_config, name=f"decoder_layers.{j}") for j in range(config.decoder_num_hidden_layers)
        ]
    ...
    def call(
        ...
        return TFViTDiffMAEDecoderOutput(logits=logits, hidden_states=all_hidden_states, attentions=all_self_attentions)
...
class TFViTDiffMAEForPreTraining(TFViTDiffMAEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.vit = TFViTDiffMAEMainLayer(config, name="vit")
        self.decoder = TFViTDiffMAEDecoder(
            config,
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(VIT_MAE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=TFViTDiffMAEForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def call(
        self,
        pixel_values: TFModelInputType | None = None,
        noise: tf.Tensor = None,
        ...
        return_dict: Optional[bool] = None,
        training: bool = False,
    ) -> Union[TFViTDiffMAEForPreTrainingOutput, Tuple[tf.Tensor]]:
        r"""
        Returns:
        ...
        ```python
        >>> from transformers import AutoImageProcessor, TFViTDiffMAEForPreTraining
        >>> from PIL import Image
        >>> import requests
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("facebook/vit-mae-base")
        >>> model = TFViTDiffMAEForPreTraining.from_pretrained("facebook/vit-mae-base")
        ...
        return TFViTDiffMAEForPreTrainingOutput(
            loss=loss,
            logits=logits,
            mask=mask,
            ids_restore=ids_restore,
